# Event Planner Agent V2.1 - Real-Time Data Integration
...
async def sync_caterers(location: str, cuisines: List[str] = None):
    """Sync caterers from Yelp for a specific location"""
    result = await data_ingestion_service.sync_caterers_for_location(location, cuisines)
    return {"success": True, "data": result}

@app.post("/sync/venues")
async def sync_venues(location: str):
    """Sync venues from Yelp for a specific location"""
    result = await data_ingestion_service.sync_venues_for_location(location)
    return {"success": True, "data": result}
```
...
└──────────────────────────┼───────────────────────────────────┘
         ...
         ▼                 ▼                 ▼
┌─────────────────┐ ┌─────────────┐ ┌──────────────────┐
│  MCP Scraper    │ │  MCP Google │ │   PostgreSQL     │
│  (Yelp API)     │ │  Maps API   │ │  (Real Data)     │
│  Port 3003      │ │  Port 3004  │ │  Port 5432       │
└─────────────────┘ └─────────────┘ └──────────────────┘
         │                 │                 │
         ▼                 ▼                 ▼
┌─────────────────┐ ┌─────────────┐ ┌──────────────────┐
│  Yelp Fusion    │ │  Google     │ │  External Data   │
│  API            │ │  Maps API   │ │  + Seed Data     │
└─────────────────┘ └─────────────┘ └──────────────────┘
```
...
### **Before (V2.0):**
- ❌ Static seeded data (10 venues, 10 caterers)
...
### **After (V2.1):**
- ✅ Real-time data from Yelp & Google Maps
- ✅ Actual ratings, reviews, and photos
- ✅ GPS coordinates for distance calculations
- ✅ Hundreds of venues/caterers per location
...
### **1. Initial Sync**
```
User → POST /sync/caterers?location=SF
  ↓
Data Ingestion Service
  ↓
MCP Scraper → Yelp API (fetch 50 caterers)
  ↓
...
```
User → POST /plan-event
  ↓
Catering Service → PostgreSQL
  ↓
...
1. ✅ Scrape real venues and caterers from Yelp
2. ✅ Enrich data with Google Maps information
3. ✅ Store in PostgreSQL with deduplication
4. ✅ Serve real-time data to users
5. ✅ Support location-based queries
6. ✅ Provide actual ratings and reviews

**This is no longer a demo - it's a real-world event planning agent!** 🎉
